                ...
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column) >
                        ...
                        <div v-if="column.type === 'action'" >
                          <!-- actions:td中所有的操作按钮 action:每个操作按钮 actionClick操作栏点击事件 -->
                            <i-button :type="action.type" size="small" @click="action.actionClick(item)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</i-button>
                        </div>
                        <label v-if="!column.type" @click="toggle(index,item)">
                            ...
                            <span v-if="column.render">{{column.render(item,column)}}</span>
                            <span v-if="!column.render">{{renderBody(item,column) }}</span>
                        </label>
                    </td>
                </tr>
            </tbody>
...
export default {
    ...
    mounted() {
        if (this.items) {
            this.dataLength = this.Length(this.items)
            this.initData(this.deepCopy(this.items), 1, null);
            console.log("递归结束",this.initItems)
            this.cloneColumns = this.makeColumns();
            this.checkGroup = this.renderCheck(this.items)
            if (this.checkGroup.length == this.dataLength) {
                this.checks = true
        ...
        console.log("array----",this.initItems);
    },
    methods: {
      // 有无多选框折叠位置优化
      iconRow() {
